...
class Loader(abc.ABC):
    ...
    def __init__(self, context: GenericContext, path: Path) -> None:
        ...
        self.config = self.context.configs[self.context_path]
    ...
    def load(self) -> LoadedProject:
        ...
        for config_file in self.context_path.glob("config.*"):
            self._track_file(config_file)
            config_mtimes[self.context_path].append(self._path_mtimes[config_file])
    ...
    def _load_external_models(
        ...
        external_models_yaml = Path(self.context_path / c.EXTERNAL_MODELS_YAML)
        deprecated_yaml = Path(self.context_path / c.EXTERNAL_MODELS_DEPRECATED_YAML)
        external_models_path = self.context_path / c.EXTERNAL_MODELS
    ...
    def _load_requirements(self) -> t.Tuple[t.Dict[str, str], t.Set[str]]:
        ...
        requirements_path = self.context_path / c.REQUIREMENTS
    ...
    def _glob_paths(
        ...
        ignored_filepaths = set(ignore_patterns) | {
            ...
            for ignored_path in glob.glob(str(self.context_path / ignore_pattern), recursive=True)
        }
...
class SqlMeshLoader(Loader):
    ...
    def _load_scripts(self) -> t.Tuple[MacroRegistry, JinjaMacroRegistry]:
        ...
        for path in self._glob_paths(
            self.context_path / c.MACROS,
            ignore_patterns=self.config.ignore_patterns,
            extension=".py",
        ):
            if import_python_file(path, self.context_path):
                self._track_file(path)
    ...
    def _load_sql_models(
        ...
        models: UniqueKeyDict[str, Model] = UniqueKeyDict("models")
        cache = SqlMeshLoader._Cache(self, self.context_path)

        for path in self._glob_paths(
            self.context_path / c.MODELS,
            ignore_patterns=self.config.ignore_patterns,
            extension=".sql",
        ):
    ...
    def _load_python_models(
        ...
        try:
            for path in self._glob_paths(
                ...
                self._track_file(path)
                import_python_file(path, self.context_path)
    ...
    def _load_materializations(self) -> None:
        """Loads custom materializations."""
        for path in self._glob_paths(
            self.context_path / c.MATERIALIZATIONS,
            ignore_patterns=self.config.ignore_patterns,
            extension=".py",
        ):
    ...
    class _Cache:
        def __init__(self, loader: SqlMeshLoader, context_path: Path):
            ...
            self._model_cache = ModelCache(self.context_path / c.CACHE)
        ...
        def _cache_entry_name(self, target_path: Path) -> str:
            return "__".join(target_path.relative_to(self.context_path).parts).replace(
                target_path.suffix, ""
            )
        ...
        def _model_cache_entry_id(self, model_path: Path) -> str:
            mtimes = [
                ...
                self._loader._config_mtimes.get(self.context_path),
