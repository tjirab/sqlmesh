...
class TerminalConsole(Console):
    ...
    def log_skipped_models(self, snapshot_names: t.Set[str]) -> None:
        if snapshot_names:
            msg = "  " + "\n  ".join([_strip_identifier_quotes(name) for name in snapshot_names])
    ...
    def log_failed_models(self, errors: t.List[NodeExecutionFailedError]) -> None:
        if errors:
            ...
            for i, error in enumerate(errors):
                ...
                node_name = _strip_identifier_quotes(node_name)
...
def _strip_identifier_quotes(identifier: str) -> str:
    for delim in ["'", '"', "[", "]", "`"]:
        identifier = identifier.replace(delim, "")
    return identifier
